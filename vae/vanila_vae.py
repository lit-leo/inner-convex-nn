...
train_loader = range(0, 1)
test_loader = range(16277, 16278)
log_interval = 1

totensor = transforms.ToTensor()
def load_batch(batch_idx, istrain):
    if istrain:
        template = './celeba/img_align_celeba/splits/train/%s.jpg'
    else:
        template = './celeba/img_align_celeba/splits/valid/%s.jpg'
    l = [str(batch_idx*10 + i).zfill(6) for i in range(1, 11)]
    data = []
    for idx in l:
        img = Image.open(template%idx).resize((128, 128))
